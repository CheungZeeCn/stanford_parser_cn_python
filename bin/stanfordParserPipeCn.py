...
model_path = os.path.join(includes_dir, "chineseFactored.ser.gz")
...
command = 'java -server -mx%dm -cp "%s" edu.stanford.nlp.parser.lexparser.LexicalizedParser '\
            '-sentences newline -outputFormat wordsAndTags,penn,typedDependencies -encoding utf-8 "%s" -'\
            % (mx, parser_path, model_path)
