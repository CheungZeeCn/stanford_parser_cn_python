...
def consume(s, pattern, flags=0, return_match=False):
  match = re.match(pattern, s, flags=flags)
  if match == None:
    if return_match:
      return s, None
    return s
...
Node = namedtuple('T', ['tag', 'children'])
...
def parse_penn(penn):
  ...
  def convert(t):
    if not isinstance(t, list):
      return t
    #return {'tag': t[0], 'children': [convert(x) for x in t[1:]]}
    return Node(t[0], [convert(x) for x in t[1:]])

  tree = convert(tree[0])

  if len(penn.strip()) != 0:
    print original_penn
    raise Exception('Could not parse Penn notation')
...
def parse_deps(deps, words):
  ...
  for relation in relations:
    ...
    r.append(Relation(
      rel,
      set([rel] + IS_A.get(rel,[])),
      lookup(match.group(2), words),
      lookup(match.group(3), words)
    ))
  return r

Word = namedtuple('W', ['index', 'word', 'tag'])

def parse_triplet(wordsandtags, penn, deps):
  words = [Word(i, wt.split('/')[0], wt.split('/')[1]) for i,wt in enumerate(wordsandtags.split(' '))]
  penn = parse_penn(penn)
  deps = parse_deps(deps, words)
  return words, penn, deps

#This data from Stefanie Tellex at MIT (http://projects.csail.mit.edu/spatial/Stanford_Parser)
...
def fill_is_a(r, past=None):
  if len(r) == 0:
    return
  for k in r:
    IS_A[k] = past or []
    fill_is_a(r[k], past=[k] + (past or []))
